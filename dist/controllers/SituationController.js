...
//cadastrar
router.post("/status-cliente", async (req, res) => {
    try {
        const data = req.body;
...
//listar
router.get("/status-cliente", async (req, res) => {
    try {
        const situationRepository = AppDataSource.getRepository(Situation);
...
//listar por id
router.get("/status-cliente/:id", async (req, res) => {
    try {
...
//atualizar
router.put("/status-cliente/:id", verificarToken, async (req, res) => {
    try {
        const id = Number(req.params.id);
        const situationRepository = AppDataSource.getRepository(Situation);
        const situation = await situationRepository.findOneBy({ id });
...
//deletar
router.delete("/status-cliente/:id", verificarToken, async (req, res) => {
    try {
        const id = Number(req.params.id);
        const situationRepository = AppDataSource.getRepository(Situation);
        const situation = await situationRepository.findOneBy({ id });

...
router.post("/login-techstore", async (req, res) => {
    try {
...
router.get("/validar-acesso", verificarToken, async (req, res) => {
    return res.status(200).json({
        messagem: "Token valido!",
        userId: req.user.id,
    });
});
router.post("/recuperar-acesso", async (req, res) => {
    try {
        const data = req.body;
        const schema = yup.object().shape({
...
router.post("/validar-chave-acesso", async (req, res) => {
    try {
        const data = req.body;
        const schema = yup.object().shape({
            recoverPassword: yup
...
router.put("/atualizar-acesso", async (req, res) => {
    try {
        const data = req.body;
        const schema = yup.object().shape({
            recoverPassword: yup

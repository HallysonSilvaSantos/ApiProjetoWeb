...
//cadastrar
router.post("/categoria-produto", async (req, res) => {
    try {
        var data = req.body;
...
//listar
router.get("/categoria-produto", async (req, res) => {
    try {
        const productCategoryRepository = AppDataSource.getRepository(ProductCategory);
...
//listar por id
router.get("/categoria-produto/:id", async (req, res) => {
    try {
...
//atualizar
router.put("/categoria-produto/:id", verificarToken, async (req, res) => {
    try {
        const id = Number(req.params.id);
        const productCategoryRepository = AppDataSource.getRepository(ProductCategory);
        const productCategory = await productCategoryRepository.findOneBy({ id });
...
//deletar
router.delete("/categoria-produto/:id", verificarToken, async (req, res) => {
    try {
        const id = Number(req.params.id);
        const productCategoryRepository = AppDataSource.getRepository(ProductCategory);
        const productCategory = await productCategoryRepository.findOneBy({ id });

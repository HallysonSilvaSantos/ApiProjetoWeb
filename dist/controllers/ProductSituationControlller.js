...
//cadastar
router.post("/status-produto", async (req, res) => {
    try {
        const data = req.body;
...
//listar
router.get("/status-produto", async (req, res) => {
    try {
        const productSituationRepository = AppDataSource.getRepository(ProductSituation);
...
//listar por id
router.get("/status-produto/:id", async (req, res) => {
    try {
...
//atualizar
router.put("/status-produto/:id", verificarToken, async (req, res) => {
    try {
        const id = Number(req.params.id);
        const productSituationRepository = AppDataSource.getRepository(ProductSituation);
        const productSituation = await productSituationRepository.findOneBy({ id });
...
//deletar
router.delete("/status-produto/:id", verificarToken, async (req, res) => {
    try {
        const id = Number(req.params.id);
        const productSituationRepository = AppDataSource.getRepository(ProductSituation);
        const productSituation = await productSituationRepository.findOneBy({ id });
